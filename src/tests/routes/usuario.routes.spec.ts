...
function createLoginToken(server: any, loginDetails: any, done: any) {
  request(server)
    .post("/login")
    .send(loginDetails)
...
describe("Testando a rota de usuários", () => {
  ...
  test("Deverá retornar um array JSON", async () => {
    const email = "novoemail@email.com";
    const senha = "123456";
    const usuarioInfo = { email, senha };

    createLoginToken(app, usuarioInfo, async function (header: any) {
      return request(app)
        .get("/usuarios")
  ...
  /* test("Deve atualizar um usuário no banco de dados", async () => {
    ...
    createLoginToken(app, usuarioLogin, async function (header: any) {
